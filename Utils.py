...
class Utils(object):
	...
	def run(*command_args, terminate_on_fail=False, replaceForeground=False):
		if os.environ.get('UTILS_DEBUG') == 'true' or os.environ.get('UTILS_DEBUG') == 'True':
			return Utils.debug(*command_args, terminate_on_fail=terminate_on_fail)
		try:
			if replaceForeground:
				os.execvp(command_args[0], command_args)
			output = subprocess.check_output(command_args, universal_newlines=True)
			if output.endswith('\n'):
				output = output[:-1]
			return output
			...
			try:
				exit_status = int(re.search("[0-9]+$", str(sys.exc_info()[1])).group(0))
